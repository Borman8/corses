...
class RailRoad
  ...
  def show_trains_list
    ...
    @trains.each { |train| puts "Поезд номер: #{train.number}, тип поезда: #{train.kind}, количество вагонов: #{train.vans}"  }
  end

  def show_trains_list_number
    @trains.each { |train| puts "Поезд номер: #{train.number}, тип #{train.kind}, в составе которого #{train.vans} вагонов(а)" }
  end
